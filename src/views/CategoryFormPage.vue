...
export default {
	...
	methods: {
		...mapActions(useCategoryStore, ["postCategory", "putCategory"]),
		handlePutorPost() {
			if (this.editFlag === true) {
				this.putCategory(this.cropData);
			} else if (this.editFlag === false) {
				this.postCategory(this.cropData);
			}
		},
		handleStatus(value) {
	...
	computed: {
		...mapState(useCategoryStore, ["categoryDetail", "editFlag"]),
		heading() {
			if (this.editFlag === true) {
				return "Edit";
			} else if (this.editFlag === false) {
